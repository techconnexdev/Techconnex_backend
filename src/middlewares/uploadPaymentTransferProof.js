...
// 10 MB per file limit for payment transfer proof images/documents
...
export const uploadPaymentTransferProof = async (req, res, next) => {
  // Use multer to handle file upload (stores in memory)
  upload.single("transferProof")(req, res, async (err) => {
    if (err) {
      return next(err);
    }
    ...
    try {
      ...
      // Fetch payment to get provider ID
      ...
      const providerId = payment.project.providerId;

      if (!providerId) {
        return next(new Error("Provider ID not found for this payment"));
      }

      // Generate R2 key with provider ID in the path
      ...
      console.log("Payment transfer proof uploaded to R2:", r2Key);
      next();
    } catch (error) {
      console.error("Error uploading payment transfer proof to R2:", error);
